...
export class WordsManagementService {
  ...
  static async updateWordArchiveStatus(
    progressId: string,
    isArchived: boolean,
  ): Promise<void> {
    const supabase = await createClient();
  ...
  static async performBulkAction(payload: BulkActionPayload): Promise<void> {
    const supabase = await createClient();
  ...
  static async getWordsStatistics(): Promise<WordsStatistics> {
    const supabase = await createClient();
