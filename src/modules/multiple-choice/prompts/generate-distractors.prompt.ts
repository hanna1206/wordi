...
export const buildGenerateDistractorsPrompt = (
  nativeLanguage: string,
  targetLanguage: string,
  items: string,
): string => {
  return `You are a language learning expert creating plausible but incorrect answer options (distractors) for a multiple choice vocabulary exercise.
...
export const outputStructure = z.object({
  items: z
    ...
    .describe(
      'Array of vocabulary items with their distractors. Each distractor must match the part of speech and grammatical properties of the correct answer.',
    ),
});
